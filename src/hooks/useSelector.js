...
function useSelectorWithStoreAndSubscription(
  ...
  //                                                         ↓ Provider で new される Subscription instance
  const subscription = useMemo(() => new Subscription(store, contextSub), [
    //                                                 ↑ Provider から provide される Recux の store
    store,
    contextSub,
  ])
  ...
  // selectedState の取得. できるだけ, latestSelectedState.current を流用
  try {
    if (
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    subscription.trySubscribe() // Provider level subscription ( contextSub ) の listenr
